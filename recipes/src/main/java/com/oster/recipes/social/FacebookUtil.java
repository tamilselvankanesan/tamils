...
public class FacebookUtil {
	...
	private String getLongLivedUserAccessToken(
		...
		try {
			...
			UriComponentsBuilder url = UriComponentsBuilder
					...
					.queryParams(params);
			ResponseEntity<JsonNode> response = rt
					.getForEntity(url.build().encode().toUri(), JsonNode.class);
	...
	public String getLongLivedPageAccessToken(String shortLivedUserAccessToken,
			String pageName) {
	...
	public JsonNode getPageDetails(String shortLivedUserAccessToken,
			String pageName) {
		String longLivedUserAccessToken = getLongLivedUserAccessToken(
				shortLivedUserAccessToken);
		String userId = getUserIdFromAccessToken(shortLivedUserAccessToken);
		UriComponentsBuilder url = UriComponentsBuilder
				.fromHttpUrl("https://graph.facebook.com/" + userId + "/accounts")
	...
	public Result<String> publishPost(DataMap pageMap, String message) {
		...
		ResponseEntity<String> response = rt.postForEntity("https://graph.facebook.com/"+pageMap.getId()+"/feed", request, String.class);
		log.info(response.getBody());
